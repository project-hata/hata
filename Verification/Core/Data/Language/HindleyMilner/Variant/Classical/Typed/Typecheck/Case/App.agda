...
module _ {𝒯 : 𝒰 𝑖} {{_ : isℒHMTypeCtx {𝑗} 𝒯}} where
  ...
  inv-app : ∀{k μs} {Q : ℒHMQuant Σ k} {Γ : ℒHMCtx Q μs} {β : ℒHMType Σ μs}
            --------------------------------------
            -- constructor inputs
            -> {te : UntypedℒHM k}
            -> {se : UntypedℒHM k}
  ...
  --   algorithm.
  module typecheck-app {νsₐ : ℒHMTypes Σ} {k : ♮ℕ} {Q : ℒHMQuant Σ k} (Γ : ℒHMCtx Q νsₐ) where
      ...
      module Success-te (𝑇-te! : PrincipalTypeAssignment Γ te) where
        ...
        module Success-se (𝑇-se! : PrincipalTypeAssignment Γ₀ se) where
          ...
          module Success-Coeq (x : hasCoequalizer (asArr u) (asArr v)) where
            ...
            π : νs₂ ⟶ ⟨ x ⟩
            π = π₌ {{isCategory:ℒHMTypeCtx}} {{of x}}

            f : νs₂ₐ ⟶ ⟨ x ⟩
            f = ι₀ ◆ π

            factor:f : isSplitEpiMonoFactorizable f
            factor:f = factorize {{_}} {{hasSplitEpiMonoFactorization:ℒHMTypeCtx}} f

            νs₃ₐ νs₃ₓ νs₃ : ℒHMTypes Σ
            νs₃ₐ = image factor:f
            νs₃ₓ = rest factor:f
            ...
            module lem-5 where abstract
              ...
              Proof = α₂ ⇃[ π ◆ ⟨ splitting factor:f ⟩⁻¹ ]⇂      ⟨ sym-Path (functoriality-◆-⇃[]⇂ {τ = α₂} {f = π} {⟨ splitting factor:f ⟩⁻¹}) ⟩-≡
                ...
                where
                  ...
                  lem-5b : α₂ ⇃[ π ]⇂ ≡ (β₂ ⇒ γ₂) ⇃[ π ]⇂
                  lem-5b = ∼→≡ lem-5a
                  -- trans-Path (trans-Path (sym-Path abstract-⇃[]⇂) (cong fromArr (≡-Str→≡ lem-5a))) abstract-⇃[]⇂
              ...
              module lem-13 where abstract
                ...
                -- Proof = ≡→≡-Str (isInjective:fromArr sublem-0)
                  where
                    ...
                    sublem-0 = (lem-11.Proof ∙-≡ sym-Path lem-12.Proof)
                      -- (abstract-⇃[]⇂ ∙-≡ (lem-11.Proof ∙-≡ sym-Path lem-12.Proof)) ∙-≡ sym-Path abstract-⇃[]⇂
