...
module Verification.Core.Theory.FirstOrderTerm.Unification.PCF.OccurFail where
...
module _ {Σ : FOSignature 𝑖} where
  ...
  mutual
    depths-𝕋× : ∀{Γ Δ} -> (t : FOTerms Σ Δ Γ) -> 人ℕ
    depths-𝕋× ◌-⧜ = 0
    depths-𝕋× (t ⋆-⧜ s) = depths-𝕋× t ⋆ depths-𝕋× s
    depths-𝕋× (incl x) = depth-𝕋× x

    depth-𝕋× : ∀{Γ τ} -> (t : FOTerm Σ Γ τ) -> 人ℕ
    depth-𝕋× (var x) = 0
    depth-𝕋× (con c x) = 1 ⋆ (depths-𝕋× x)

  module §-depth-𝕋× where
    mutual
      prop-1s : ∀{Γ Δ} (t : FOTerms Σ Δ Γ) -> ∀{Γ' : ⧜𝐒𝐮𝐛𝐬𝐭 (term-FO Σ)} -> (σ : incl (incl Γ) ⟶ ι Γ')
              -> (depths-𝕋× t) ≤ (depths-𝕋× (reext-FOTerms ⟨ σ ⟩ t))
      prop-1s ◌-⧜ σ = reflexive
      prop-1s (t ⋆-⧜ s) σ = prop-1s t σ ⩚⋆⩚ prop-1s s σ
      prop-1s (incl x) σ = prop-1 x σ

      prop-1 : ∀{Γ τ} (t : FOTerm Σ Γ τ) -> ∀{Γ' : ⧜𝐒𝐮𝐛𝐬𝐭 (term-FO Σ)} -> (σ : incl (incl Γ) ⟶ ι Γ')
              -> (depth-𝕋× t) ≤ (depth-𝕋× (reext-term-FO ⟨ σ ⟩ _ t))
      prop-1 (var x) σ = initial-◌-≤
      prop-1 (con c x) σ = reflexive ⩚⋆⩚ prop-1s x σ

    mutual
      prop-2s : ∀{Γ Δ τ'} (t : FOTerms Σ Δ Γ) (v : Γ ∍ τ') (occ : (VarPath-FOTerms Σ t v)) -> ∀{Γ' : ⧜𝐒𝐮𝐛𝐬𝐭 (term-FO Σ)} -> (σ : incl (incl Γ) ⟶ ι Γ')
              -> (depths-𝕋× t) ⋆ (depth-𝕋× (⟨ σ ⟩ _ v)) ≤ (depths-𝕋× (reext-FOTerms ⟨ σ ⟩ t))
      ...
      prop-2 : ∀{Γ τ τ'} (t : FOTerm Σ Γ τ) (v : Γ ∍ τ') (occ : (VarPath-Term-𝕋× Σ t v)) -> ∀{Γ' : ⧜𝐒𝐮𝐛𝐬𝐭 (term-FO Σ)} -> (σ : incl (incl Γ) ⟶ ι Γ')
              -> (depth-𝕋× t) ⋆ (depth-𝕋× (⟨ σ ⟩ _ v)) ≤ (depth-𝕋× (reext-term-FO ⟨ σ ⟩ _ t))
  ...
  module _ {Γ τ} (t : FOTerm Σ Γ τ) (v : Γ ∍ τ) (occ : (VarPath-Term-𝕋× Σ t v)) {d} (pd : depth-𝕋× t ∼ 1 ⋆ d) where
    -- module _ {σ : ⧜𝐒𝐮𝐛𝐬𝐭 (Terms Σ)} {{_ : isCoequalizer (incl t) (simpleVar v) σ}} where

    module §-Occur-𝕋× {Γ' : ⧜𝐒𝐮𝐛𝐬𝐭 (term-FO Σ)} {{_ : isCoequalizerCandidate (map (⧜subst (incl t))) (map (simpleVar v)) (ι Γ')}} where
      ...
      private
        ...
        lem-1 : reext-term-FO ⟨ σ ⟩ _ t ≡ val
        lem-1 = (funExt⁻¹ (⟨ equate-π₌? ⟩ _)) incl

        lem-2 : depth-𝕋× (reext-term-FO ⟨ σ ⟩ _ t) ≡ depth-𝕋× val
        lem-2 = cong depth-𝕋× lem-1

        lem-3 : depth-𝕋× t ⋆ depth-𝕋× val ≤ depth-𝕋× (reext-term-FO ⟨ σ ⟩ _ t)
        ...
        lem-4 = (1 ⋆ d) ⋆ depth-𝕋× val             ⟨ 命reflexive (pd ⁻¹) ⩚⋆⩚ reflexive ⟩-≤
                ...
                depth-𝕋× (reext-term-FO ⟨ σ ⟩ _ t)  ⟨ 命reflexive (fromPath lem-2) ⟩-≤
